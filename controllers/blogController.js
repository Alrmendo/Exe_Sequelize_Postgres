...
controller.showList = async (req, res) => {
    ...
    // neu nguoi dung co chon category thi loc blog theo category
    if (category) {
        options.where.categoryId = category
    }

    // neu nguoi dung co chon tag thi loc blog theo tag
    if (tag) {
        options.include.push({ model: models.Tag, where: { id: tag } })
    }

    // neu nguoi dung co nhap thi loc blog theo keyword
    if (keyword.trim() != "") {
        options.where[Op.or] = {
    ...
    //dem co bao nhieu block thoa dieu kien
